<script type="text/javascript">
	...
	function tampildata()
	{
		$("#myTable").DataTable({
	        ...
	        "serverSide": false,
	        "ajax" :{
	          url : "<?= site_url('cpanel/master-data/tag/ajax') ?>",
	          type: "POST",
	          dataType: "json",
	        },
	        "columns" : [{ "data" : "nomor" }, 
	...
	$('body').on('click', '.add', function(event) {
	    ...
	    $.ajax({
	      url: '<?= site_url('cpanel/master-data/tag/modal-add') ?>',
	      type: 'post',
	      dataType: 'html',
	      beforeSend: function(){   
	...
	$('body').on('submit', '#simpan', function(event) {
      ...
      $.ajax({
        url: '<?= site_url('cpanel/master-data/tag/simpan') ?>',
        type: 'post',
        dataType: 'html',
...
$('body').on('click', '.edit', function(event) {
  ...
  /* Act on the event */
    ...
    $.ajax({
      url: '<?= site_url('cpanel/master-data/tag/modal-edit') ?>',
      type: 'post',
      dataType: 'html',
      data: {id: id,},
      beforeSend: function(){   
 ...
 $('body').on('submit', '#perbarui', function(event) {
    ...
    $.ajax({
      url: '<?= site_url('cpanel/master-data/tag/perbarui') ?>',
      type: 'post',
      dataType: 'html',
      data: new FormData(this),
      contentType: false,       
  ...
  $('body').on('click', '.delete', function(event) {
    ...
    $.ajax({
      url: '<?= base_url('cpanel/master-data/tag/delete') ?>',
      type: 'post',
      dataType: 'html',
      data: {id:id,},
      beforeSend: function(){   
 ...
 $('body').on('click', '.hapus_data', function(event) {
   ...
   /* Act on the event */
    ...
    $.ajax({
        url: '<?= base_url('cpanel/master-data/tag/hapus') ?>',
        type: 'post',
        dataType: 'html',
        data: {id:id,},
        beforeSend: function(){   
